...
export const groupAddUserToPath: FtOperation = {
	resource: 'group',

	options: {
		name: 'Add User to Group Path',
		value: 'groupAddUserToPath',
		action: 'Add user to group path',
		description: 'Add user to all groups and subgroups matching a path',
	},
	...
	handler: async (
		...
		const selectedGroups: GroupUserModel[] = selectGroupsMatchingPath(response.groupTreeList, groupPathValue.split("/").filter(s => s.length > 0));

		// add user to groups
		for (const selectedGroup of selectedGroups) {
			await client.groupAddUsers(realmNameValue, selectedGroup.id, [userIdValue]);
		}
